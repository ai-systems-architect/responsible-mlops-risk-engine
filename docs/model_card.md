...
- **Global:** SHAP beeswarm plot generated by `src/training/evaluate.py`
  after every evaluation run — saved to `docs/shap_summary.png`.
  Shows each feature's contribution direction and magnitude across all
  test records using `shap.TreeExplainer` (exact method for gradient
  boosted trees, not an approximation).
- **Per-record:** SHAP waterfall rendered inline in the Streamlit demo
...
- Decision log: DL-001 through DL-018 — `docs/decision_log.md`
